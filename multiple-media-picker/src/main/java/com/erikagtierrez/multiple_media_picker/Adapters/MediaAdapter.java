...
public class MediaAdapter extends RecyclerView.Adapter<MediaAdapter.MyViewHolder>{
    ...
    public void onBindViewHolder(MyViewHolder holder, int position) {
        Glide.with(context).load("file://"+bitmapList.get(position)).override(153,160).transition(withCrossFade()).centerCrop().dontAnimate().skipMemoryCache(true).into(holder.thumbnail);
