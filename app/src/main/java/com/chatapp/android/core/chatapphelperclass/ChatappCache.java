...
public class ChatappCache {
    private static final String TAG = "ChatappCache";
    private LruCache<String, Bitmap> mMemoryCache;

    /**
     * Creating a new ChatappCache object using the specified parameters.
     *
     * @param memCacheSizePercent The cache size as a percent of available app memory.
     */
    private ChatappCache(float memCacheSizePercent) {
        init(memCacheSizePercent);
    }

    /**
     * Find and return an existing ChatappCache stored in a {@link RetainFragment}, if not found a new
     * one is created using the supplied params and saved to a {@link RetainFragment}.
     *
     * @param fragmentManager The fragment manager to use when dealing with the retained fragment.
     * @param memCacheSizePercent The cache size as a percent of available app memory.
     * @return An existing retained ChatappCache object or a new one if one did not exist
     */
    public static ChatappCache getInstance(
            FragmentManager fragmentManager, float memCacheSizePercent) {

        // Search for, or create an instance of the non-UI RetainFragment
        final RetainFragment mRetainFragment = findOrCreateRetainFragment(fragmentManager);

        // See if we already have an ChatappCache stored in RetainFragment
        ChatappCache imageCache = (ChatappCache) mRetainFragment.getObject();

        // No existing ChatappCache, create one and store it in RetainFragment
        if (imageCache == null) {
            imageCache = new ChatappCache(memCacheSizePercent);
    ...
    public static int getBitmapSize(Bitmap bitmap) {
        if (ChatappUtils.hasHoneycombMR1()) {
     ...
     * changes. It will be used to retain the ChatappCache object.
     */
